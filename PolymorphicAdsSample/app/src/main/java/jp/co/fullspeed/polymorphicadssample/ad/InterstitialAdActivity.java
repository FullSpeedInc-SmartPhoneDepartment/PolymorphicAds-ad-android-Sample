...
/**
 * インタースティシャル広告を利用したサンプルアクティビティ.
 */
public class InterstitialAdActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.interstitial_sample_activity);

        // （任意） 広告カスタマイザー生成
        PolymorphicAdsCustomizer customizer = new PolymorphicAdsCustomizer(this, PolymorphicAdsCustomizer.AdType.INTERSTITIAL);
        customizer.setIntervalTime(0);
        customizer.setSkipCount(0);

        // ① 広告ユニット初期化
        PolymorphicAds.init(this, AdUnitIdConst.INTERSTITIAL_AD_UNIT_ID, PolymorphicAds.AdType.INTERSTITIAL, customizer);

        // ② 広告ロード（Ad情報取得）
        PolymorphicAds.load(AdUnitIdConst.INTERSTITIAL_AD_UNIT_ID);

        findViewById(R.id.show_interstitial_button).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // ③ 広告表示（load()の後であればいつCallしても良い）
                if (!PolymorphicAds.isReady(AdUnitIdConst.INTERSTITIAL_AD_UNIT_ID)) {
                    // 広告の表示準備が未完の場合
                    Toast.makeText(InterstitialAdActivity.this, getString(R.string.now_preparing), Toast.LENGTH_SHORT).show();
                    return;
                }
                if (PolymorphicAds.isDisplay(AdUnitIdConst.INTERSTITIAL_AD_UNIT_ID)) {
                    // 広告がすでに表示されている場合
                    Toast.makeText(InterstitialAdActivity.this, getString(R.string.now_on_display), Toast.LENGTH_SHORT).show();
                    return;
                }
                PolymorphicAds.show(AdUnitIdConst.INTERSTITIAL_AD_UNIT_ID);
            }
        });
        findViewById(R.id.show_interstitial_button_with_force_close).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // ③ 広告表示（load()の後であればいつCallしても良い）
                if (!PolymorphicAds.isReady(AdUnitIdConst.INTERSTITIAL_AD_UNIT_ID)) {
                ...
                new AsyncTask<Void, Void, Void>() {
                    ...
                    protected void onPostExecute(Void aVoid) {
                        ...
                        PolymorphicAds.forceClose(AdUnitIdConst.INTERSTITIAL_AD_UNIT_ID);
                        Toast.makeText(InterstitialAdActivity.this, getString(R.string.perform_force_close), Toast.LENGTH_SHORT).show();
                    }
                }.execute();
            }
        });
    ...
    protected void onResume() {
        ...
        PolymorphicAds.setCallback(
                AdUnitIdConst.INTERSTITIAL_AD_UNIT_ID,
                this,
